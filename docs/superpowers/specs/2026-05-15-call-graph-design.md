...
## 11. v3 Changes — Structured Storage + LLM Consumption (2026-05-26)

The v3 iteration transforms Layer 3 from a single-output pipeline (Mermaid + Markdown) to a three-way persistent storage system (SQLite + JSON + ChromaDB) designed for efficient LLM consumption.

### 11.1 Motivation

The Mermaid diagrams and Markdown documents serve two purposes:
1. **Dependency query database** — reduce LLM token consumption by enabling structured queries instead of full graph loading
2. **Persistent memory** — maintain project knowledge across LLM conversations

The previous architecture rebuilt NetworkX graphs on every `analyze_deps` call. For a project with 1000+ nodes and 2000+ edges, this is wasteful.
...
```
   ...
   dependency-    dependency-  dependency_
   graph.db       graph.json   modules collection
         │
         ▼
   analyze_deps (SQLite 优先查询, 毫秒级)
```

### 11.3 SQLite Schema (`DependencyStore`)

**File:** `src/ai_code2doc/analyzer/dependency_store.py`

| Table | Key Fields | Purpose |
|-------|-----------|---------|
| `nodes` | id (PK), path, name, kind, file_hash | File and symbol nodes |
| `edges` | source_id, target_id, edge_type, confidence, weight, line_number | Import, call, contains relationships |
| `metadata` | key (PK), value | Version, timestamps |

Indexes: `idx_edges_source`, `idx_edges_target`, `idx_edges_type`, `idx_nodes_kind`, `idx_nodes_path`.

### 11.4 JSON Export

Full dump of SQLite data to `dependency-graph.json`:

```json
{
  "version": "2.0",
  "generated_at": "2026-05-26T...",
  "stats": { "nodes": 1028, "edges": 2139, "resolved_calls": 1129 },
  "nodes": [...],
  "edges": [...]
}
```

Used for: git diff, debugging, tool interop.

### 11.5 Module-Level Vector Embeddings

Instead of embedding every node individually, module-level summaries are generated by grouping files by parent directory. Each summary includes:
- Module path, file count, file names
- Top dependencies (by import edge count)
- Top dependents (by import edge count)
- Resolved outgoing call count

Stored in a dedicated ChromaDB collection: `dependency_modules`.

### 11.6 Enhanced analyze_deps Tool

**New query modes:**

| Mode | Description | Example |
|------|-------------|---------|
| `subgraph` | All edges for a set of files | `mode=subgraph, files=a.py,b.py,c.py` |
| `impact` | BFS blast radius to configurable depth | `mode=impact, target=X, depth=3` |
| `path` | BFS shortest dependency path between two nodes | `mode=path, target=A, end=B` |

**New parameter:** `confidence_threshold` (default: 0.80) — filters call edges by minimum confidence.

**Query path:** SQLite first (millisecond queries), fallback to NetworkX graph rebuild.

### 11.7 Incremental Updates

The system tracks `file_hash` (BLAKE2b) for each file node:
1. On each run, compute hashes for all source files
2. Compare against stored hashes via `get_changed_files()`
3. Only re-process changed files
4. Delete old edges for changed files, insert new ones
5. Unchanged files' data remains untouched
...
.ai_code2doc/
├── layer3/
│   ├── dependency-graph.md      # Human-readable (retained)
│   ├── dependency-graph.mmd      # Mermaid diagram (retained)
│   ├── dependency-graph.json     # NEW: structured export
│   └── dependency-graph.db       # NEW: SQLite query backend
└── chroma/
    └── code_knowledge/
        └── dependency_modules/    # NEW: module embedding collection
```
