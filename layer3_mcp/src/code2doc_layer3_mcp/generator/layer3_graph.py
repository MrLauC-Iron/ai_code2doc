...
# ---------------------------------------------------------------------------
# Prompt template (inlined — only layer3 needed)
# ---------------------------------------------------------------------------

_PROMPT_LAYER3 = """\
You are a dependency analysis expert. Analyze the following dependency graph and provide insights.
...
def _format_layer3_prompt(mermaid_graph: str, cycles: str, metrics: str) -> str:
    return _PROMPT_LAYER3.format(
        mermaid_graph=mermaid_graph, cycles=cycles, metrics=metrics,
    )


class Layer3GraphGenerator:
    ...
    async def _generate_llm_content(
        ...
        prompt = _format_layer3_prompt(
            mermaid_graph=mermaid_graph,
            cycles=cycles_text,
            metrics=metrics_text,
        )
        try:
            from code2doc_core.llm.client import LLMClient
